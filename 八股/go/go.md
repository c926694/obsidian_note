...
调度逻辑是这样的，**M必须绑定P才能执行G**。每个**P维护一个自己的本地G队列**，它由**runnext（容量1，最高优先级）** 和**容量256的本地队列**两部分组成，M从自己绑定的P的本地队列取G执行。当本地队列空时，M会**按优先级**依次从**全局队列**、**网络轮询器（netpoll）**、**其他P的队列**中偷取goroutine，这是**work-stealing**机制。

就是这个模型让Go能在少量线程上调度海量goroutine，是Go高并发的基础。

## 调度策略

G 让出 M、重新进入调度流程，主要有以下四种情况：

- **主动调度**：G 主动调用 `runtime.Gosched()`，让出当前执行权，调度器选择其他可运行的 G。
- **被动调度**：G 等待 channel、互斥锁、网络 IO 或执行阻塞操作，暂时无法继续运行，调度器切换到其他 G。
- **正常调度**：G 执行完成或调用 `goexit` 退出，调度器选择下一个可运行的 G。
- **抢占调度**：G 长时间占用 CPU 时，运行时请求暂停它，让出 P，避免单个 G 长时间阻塞其他 G。

- **sysmon** 是 Go runtime 启动的**后台监控线程**。它不需要绑定 P，所以即使所有 P 都被忙碌的 goroutine 占住了，sysmon 仍然能跑。

当 `sysmon` 发现某个 G 长时间运行时，会发起抢占请求。**10ms 是常用的经验阈值，不代表达到 10ms 就一定立即切换**；G 最终会在运行时认为安全的位置暂停，并重新进入调度流程。

**Go 1.14 之前**主要是**协作式抢占**：

- runtime 发出抢占请求
- G 运行到函数调用等检查点时检查抢占标记
- G 配合让出 M

因此，某些没有函数调用的死循环可能长时间不让出 M。

**Go 1.14 之后**支持**基于信号的异步抢占**：

runtime 可以向执行 G 的 M 发送信号，触发异步抢占；运行时会在安全点暂停 G，并将执行权交回调度器。它改善了长时间运行代码和死循环对调度的影响，但不是在任意机器指令位置强行暂停。

## M 获取 G 的过程

M 必须绑定 P 才能执行 G。M 通常按以下顺序寻找可运行的 G：

1. **本地队列**：从当前绑定 P 的 LRQ 中获取 G，通常可以无锁完成。
2. **全局队列**：本地队列为空时，从 GRQ 获取 G，需要竞争全局锁。
3. **网络轮询器**：检查 netpoll 中因网络 IO 就绪的 G。
4. **工作窃取**：随机选择其他 P，从其 LRQ 中窃取一部分 G，通常是大约一半。

如果仍然没有可运行的 G，M 可能短暂进入**自旋状态**继续尝试；持续没有任务后，M 会休眠，等新的 G 到来时再被唤醒。
